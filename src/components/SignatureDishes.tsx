      ...
      <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {signatureDishes.map((d, i) => (
          <motion.div
            ...
            whileHover={{ y: -4 }}
            className="ornate-card relative overflow-hidden p-6"
          >
            <div className="text-xs font-medium uppercase tracking-wider text-brand-gold-dark">
              {d.category}
            </div>
            <h3 className="mt-2 font-display text-xl text-brand-red">{d.name}</h3>
            {d.description && (
              <p className="mt-2 text-sm text-brand-ink/70">{d.description}</p>
            )}
